...
RISK_CHECK_OPTIONS = [
    {'key': 'claim',    'label': 'クレーム',   'char': 'ク',
     'hint': '罵倒・「最悪」「許せない」等の激しい言葉遣いや、特定個人への強い非難を伴う苦情。'
             '制度上の課題や問題点を冷静に指摘しているだけの回答（このアンケートの主旨に沿った通常の意見）は含めない。'},
    {'key': 'personal', 'label': '個人情報',   'char': '個', 'hint': '個人名・メールアドレス・電話番号・詳細住所'},
    {'key': 'org',      'label': '団体情報',   'char': '団',
     'hint': '学校名・病院名・施設名としての具体的な固有名詞（例：○○第一小学校、△△総合病院）が明記されている場合のみ該当。'
             '「校長」「教頭」「教務」「担任」「管理職」「ICT担当」など、役職・立場・職務内容を表す言葉がどれだけ出てきても、'
             'それ単体では絶対に該当としない（具体的な固有名詞が一切ない回答は、内容にかかわらず該当なし）。'},
